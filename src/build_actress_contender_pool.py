"""
Build the live Best Actress contender pool for a given year.
...
6. Write output/future_actress_predictions_{year}.csv
...
# Genre IDs that disqualify a film from acting award contention
EXCLUDED_GENRES = {
    16,     # Animation — voice roles
    99,     # Documentary — not eligible for acting Oscars
...
def load_film_pool(year: int) -> pd.DataFrame:
    ...
    if not path.exists():
        print(f"[actress] No BP pool at {path}, returning empty DataFrame")
    ...
    if "genre_ids" in df.columns:
        ...
        if removed:
            print(f"[actress] Excluded {removed} films with Animation/Documentary/Music genres.")

    if "best_picture_probability" in df.columns:
        df = df.sort_values("best_picture_probability", ascending=False).head(40)
        print(f"[actress] Filtered film pool to top {len(df)} prestige contenders.")
...
def load_tmdb_credits(year: int) -> pd.DataFrame:
    ...
    if not path.exists():
        return pd.DataFrame(columns=["tmdb_id", "lead_actress_name", "lead_actress_id", "lead_actress_profile_url"])
...
def load_person_manifest(year: int) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    return df[(df["year_film"] == year) & (df["category"] == "actress")].copy()


def load_current_precursors(year: int) -> pd.DataFrame:
    frames = []
    for fname, nom_col, win_col in [
        ("sag_actress_recent.csv", "sag_nom", "sag_win"),
        ("globe_actress_recent.csv", "globe_nom", "globe_win"),
        ("bafta_actress_recent.csv", "bafta_nom", "bafta_win"),
    ]:
        path = RAW_DIR / fname
        if not path.exists():
            continue
...
def build_actress_pool(year: int) -> pd.DataFrame:
    ...
    if tmdb_credits.empty and not has_precursor_data and not has_manifest:
        print(f"[actress] No TMDb credits, precursor data, or manifest for {year} — skipping.")
        print(f"[actress] Run: python src/pull_person_credits.py --year {year}")
        return pd.DataFrame()

    candidates = []

    # Primary source: TMDb credits
    if not tmdb_credits.empty and "lead_actress_name" in tmdb_credits.columns:
        merged = film_pool.merge(
            tmdb_credits[["tmdb_id", "lead_actress_name", "lead_actress_id", "lead_actress_profile_url"]],
            on="tmdb_id", how="inner"
        )
        merged = merged.dropna(subset=["lead_actress_name"])
        for _, row in merged.iterrows():
            if pd.isna(row.get("lead_actress_name")) or not str(row["lead_actress_name"]).strip():
                continue
            candidates.append({
                "name": str(row["lead_actress_name"]).strip(),
                "film": str(row["title"]).strip(),
                "tmdb_person_id": row.get("lead_actress_id", ""),
                "profile_url": row.get("lead_actress_profile_url", ""),
    ...
    if has_manifest:
        ...
        for _, row in manifest.iterrows():
            ...
            print(f"[actress] Added from manifest: {name} / {film}")
    ...
    if not candidates:
        print(f"[actress] No candidates found for year {year}")
        return pd.DataFrame()
...
def run(year: int):
    print(f"[actress] Building Best Actress contender pool for {year}...")
    pool_df = build_actress_pool(year)

    if pool_df.empty:
        print(f"[actress] Empty pool — no output written.")
        return

    print(f"[actress] Scoring {len(pool_df)} candidates...")
    scored = score_category_year(year, pool_df, category="actress")
    ...
    out_path = OUTPUT_DIR / f"future_actress_predictions_{year}.csv"
    prev_path = OUTPUT_DIR / f"future_actress_predictions_{year}_prev.csv"
    ...
    print(f"[actress] Wrote {len(scored)} contenders to {out_path}")
